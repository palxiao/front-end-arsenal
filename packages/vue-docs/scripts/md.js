...
createHtml(changelog, `<script>export default {name: 'ChangeLog'}</script>`, 'ChangeLog')
...
function createHtml(file, data, saveName) {
  const result = md.render(fs.readFileSync(file, 'utf-8').split('### 计划')[0].replace('[English](./README.en.md) | **中文**', ''))
  ...
  const html = result
    ...
    .replace(`<p><a href='https://meetqy.github.io/vite-vue3-docs/#/docs' traget='_blank'>在线体验</a></p>`, '')
  fs.writeFileSync(path.join(root, `./dist/template/${saveName}.vue`), `<template><section>${html}</div></section></template>${data}`)
}
