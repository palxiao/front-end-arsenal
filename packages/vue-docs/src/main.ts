...
import { getPropsByObject, getAstValue, getEmitsByArray, getEmitsByObject, getMethodsByObject, getSlotsByTemplate } from './ast'
import { toLine } from './utils'
import { Component, Emit, Method, Prop, RenderData } from './type'

// 返回code信息
export function vueToJsonData(code: string): { content: RenderData; component: Component } | null {
  ...
  ast.properties.map((vueParams) => {
    ...
    if (t.isObjectProperty(vueParams)) {
      ...
      switch (name) {
        // 组件名称
        case 'name': {
          componentName = toLine(getAstValue(vueParams.value)).toLocaleLowerCase()
          break
        }
  ...
  if (slots && slots.length) {
    json.slots = {
      ...
      table: {
        headers: ['名称', '说明', '返回参数'],
        rows: slots.map((item) => {
          return [item.name as string, item.desc || '-', item.params?.length ? item.params.join(',') : '-']
        }),
      },
    }
  }
  ...
  if (props && props.length) {
    json.props = {
      ...
      table: {
        ...
        rows: props.map((item) => {
          return [item.name as string, item.notes || '-', item.type as string, item.default || '-', item.required ? 'true' : 'false']
        }),
      },
    }
  }
